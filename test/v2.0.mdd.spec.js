...
describe('Mdict', () => {
  describe('oale8.mdd', () => {
    const mdict = new Mdict('mdx/testdict/oale8.mdd');

    it('#lookup>\\uk_pron.png', () => {
      const def = mdict.lookup('\\uk_pron.png');
    ...
    it('#lookup>\\us_pron.png', () => {
      const def = mdict.lookup('\\us_pron.png');
      assert.isNotEmpty(def.definition);
      assert.equal(
        def.keyText,
        '\\us_pron.png',
    ...
    it('#lookup>\\thumb\\ragdoll.jpg', () => {
      const def = mdict.lookup('\\thumb\\ragdoll.jpg');
      assert.isNotEmpty(def.definition);
      assert.equal(
        def.keyText,
        '\\thumb\\ragdoll.jpg',
    ...
    it('#lookup>\\us\\zebra__us_2.mp3', () => {
      const def = mdict.lookup('\\us\\zebra__us_2.mp3');
      assert.isNotEmpty(def.definition);
      assert.equal(
        def.keyText,
        '\\us\\zebra__us_2.mp3',
