...
var _bufferToArraybuffer = _interopRequireDefault(
  require("buffer-to-arraybuffer")
);
  ...
  (0, _createClass2["default"])(MDictBase, [
    {
      ...
      value: function _readHeader () {
        ...
        var headerByteSize = _common["default"].readNumber(
          header_size_buffer,
          _common["default"].NUMFMT_UINT32
        );

        // [4:header_byte_size + 4] header_bytes
        var headerBuffer = _readChunk["default"].sync(
          this.fname,
          4,
          headerByteSize
        );
        ...
        var headerText = _common["default"].readUTF16(
          headerBuffer,
          0,
          headerByteSize - 2
        );
    ...
    },
    {
      ...
      value: function _readKeyHeader () {
        ...
        var keyHeaderBuff = this._readBuffer(
        ...
        var keyBlockNumBuff = keyHeaderBuff.slice(
          ofset,
          ofset + this._numWidth
        );
        this.keyHeader.keyBlocksNum = _common["default"].readNumber(
          keyBlockNumBuff,
          this._numFmt
        );
        ofset += this._numWidth;
        ...
        this.keyHeader.entriesNum = _common["default"].readNumber(
          entriesNumBuff,
          this._numFmt
        );
        ofset += this._numWidth;
        ...
        if (this._version >= 2.0) {
          ...
          var keyBlockInfoDecompBuff = keyHeaderBuff.slice(
            ofset,
            ofset + this._numWidth
          );
          var keyBlockInfoDecompSize = _common["default"].readNumber(
        ...
        var keyBlockInfoSizeBuff = keyHeaderBuff.slice(
          ofset,
          ofset + this._numWidth
        );
        var keyBlockInfoSize = _common["default"].readNumber(
        ...
        var keyBlocksTotalSizeBuff = keyHeaderBuff.slice(
          ofset,
          ofset + this._numWidth
        );
        var keyBlocksTotalSize = _common["default"].readNumber(
    ...
    },
    {
      ...
      value: function _readKeyBlockInfo () {
        ...
        var keyBlockInfoBuff = this._readBuffer(
    ...
    },
    {
      ...
      value: function _decodeKeyBlockInfo (keyBlockInfoBuff) {
        ...
        if (this._version >= 2.0) {
          ...
          kbInfoBuff = pako.inflate(
            kbInfoCompBuff.slice(8, kbInfoCompBuff.length)
          );
          ...
          (0, _assert["default"])(
            ...
            "key_block_info decompress size "
              .concat(
                this.keyHeader.keyBlockInfoDecompSize,
                " should equal to keyblock info buffer length "
              )
              .concat(kbInfoBuff.length)
          );
        } else {
        ...
        while (kbCount < keyBlockNum) {
          ...
          var currKBEntriesCount = _common["default"].readNumber(
            kbInfoBuff.slice(i, i + this._numWidth),
            this._numFmt
          );
          i += this._numWidth;
          var firstKeySize = _common["default"].readNumber(
          ...
          var firstKey = this._decoder.decode(
            firstKeyBuf.slice(0, firstKeyBuf.length - termSize)
          );
          i += stepGap;

          // text tail
          var lastKeySize = _common["default"].readNumber(
            kbInfoBuff.slice(i, i + byteWidth),
            byteFmt
          );
          i += byteWidth;
          ...
          var lastKey = this._decoder.decode(
            lastKeyBuf.slice(0, lastKeyBuf.length - termSize)
          );
          i += stepGap;

          // key block compressed size
          var kbCompSize = _common["default"].readNumber(
            kbInfoBuff.slice(i, i + this._numWidth),
            this._numFmt
          );
          i += this._numWidth;
          ...
          var kbDecompSize = _common["default"].readNumber(
            kbInfoBuff.slice(i, i + this._numWidth),
            this._numFmt
          );
          i += this._numWidth;
        ...
        (0, _assert["default"])(
          ...
          "the number_entries "
            .concat(num_entries, " should equal the count_num_entries ")
            .concat(countEntriesNum)
        );
        (0, _assert["default"])(
          kbCompSizeAccu === this.keyHeader.keyBlocksTotalSize
        );
    ...
    },
    {
      ...
      value: function _decodeKeyBlock () {
        ...
        var kbCompBuff = this._readBuffer(
        ...
        for (var idx = 0; idx < this.keyBlockInfoList.length; idx++) {
          ...
          var kbCompType = new _bl["default"](
            kbCompBuff.slice(start, start + 4)
          );
          ...
          if (kbCompType.toString("hex") == "00000000") {
            ...
            var keyBlock = _lzoWrapper["default"].decompress(
              _common["default"].appendBuffer(
                header,
                kbCompBuff.slice(start + 8, end)
              ),
              decompressed_size,
              1308672
            );
            key_block = (0, _bufferToArraybuffer["default"])(keyBlock).slice(
              keyBlock.byteOffset,
            ...
            throw Error(
              "cannot determine the compress type: ".concat(
                kbCompType.toString("hex")
              )
            );
          }
          var splitedKey = this._splitKeyBlock(
            new _bl["default"](key_block),
            idx
          );
    ...
    },
    {
      ...
      value: function _splitKeyBlock (keyBlock, keyBlockIdx) {
        ...
        while (keyStartIndex < keyBlock.length) {
          ...
          // 0.2656s
          var recordStartOffset = _common["default"].readNumber(
          ...
          var keyText = this._decoder.decode(
    ...
    },
    {
      ...
      value: function _decodeRecordHeader () {
        ...
        var ofset = 0;
        var recordBlocksNum = _common["default"].readNumber(
          rhBuff.slice(ofset, ofset + this._numWidth),
          this._numFmt
        );
        ofset += this._numWidth;
        var entriesNum = _common["default"].readNumber(
          rhBuff.slice(ofset, ofset + this._numWidth),
          this._numFmt
        );
        (0, _assert["default"])(entriesNum === this.keyHeader.entriesNum);
        ofset += this._numWidth;
        var recordBlockInfoCompSize = _common["default"].readNumber(
          rhBuff.slice(ofset, ofset + this._numWidth),
          this._numFmt
        );
        ofset += this._numWidth;
        var recordBlockCompSize = _common["default"].readNumber(
          rhBuff.slice(ofset, ofset + this._numWidth),
          this._numFmt
        );
        this.recordHeader = {
    ...
    },
    {
      ...
      value: function _decodeRecordInfo () {
        ...
        for (var i = 0; i < this.recordHeader.recordBlocksNum; i++) {
          var compSize = _common["default"].readNumber(
            riBuff.slice(ofset, ofset + this._numWidth),
            this._numFmt
          );
          ofset += this._numWidth;
          var decompSize = _common["default"].readNumber(
            riBuff.slice(ofset, ofset + this._numWidth),
            this._numFmt
          );
          ofset += this._numWidth;
        ...
        (0, _assert["default"])(
          ofset === this.recordHeader.recordBlockInfoCompSize
        );
        (0, _assert["default"])(
          compAccu === this.recordHeader.recordBlockCompSize
        );
    ...
    },
    {
      ...
      value: function _decodeRecordBlock () {
        ...
        for (var idx = 0; idx < this.recordBlockInfoList.length; idx++) {
          ...
          if (rbCompType.toString("hex") === "00000000") {
            ...
            if (rbCompType.toString("hex") === "01000000") {
              ...
              recordBlock = _lzoWrapper["default"].decompress(
                _common["default"].appendBuffer(header, blockBufDecrypted),
                decompSize,
                1308672
              );
              recordBlock = (0, _bufferToArraybuffer["default"])(
                recordBlock
              ).slice(
        ...
        (0, _assert["default"])(
          sizeCounter === this.recordHeader.recordBlockCompSize
        );
    ...
    },
    {
      ...
      value: function _decode_record_block_by_rb_id (
        rbid,
        keyText,
        start,
        nextStart
        ...
        var decompAccumulator = this.recordBlockInfoList[rbid]
          .decompAccumulator;
    ...
    },
    {
      ...
      value: function _strip_key_or_ingore_case () {
        return function (key) {
          ...
          if (this._isStripKey()) {
            key = key.replace(
              _common["default"].REGEXP_STRIPKEY[this.ext],
              "$1"
            );
          }
          if (!this._isKeyCaseSensitive()) {
    ...
    },
    {
      ...
      value: function _isKeyCaseSensitive () {
        return (
          ...
          _common["default"].isTrue(this.header.isCaseSensitive)
        );
      },
    },
    {
      ...
      value: function _isStripKey () {
        return (
          this.options.isStripKey ||
          _common["default"].isTrue(this.header.StripKey)
        );
      },
