...
var Mdict = function (_MdictBase) {
  ...
  _createClass(Mdict, [{
    ...
    value: function _stripKey() {
      var keyCaseSensitive = this.searchOptions.keyCaseSensitive || isTrue(this.header.KeyCaseSensitive);
      var stripKey = this.searchOptions.stripKey || isTrue(this.header.stripKey);
      ...
      return this.searchOptions.stripKey || isTrue(this.header.stripKey || (this._version >= 2.0 ? "" : "yes")) ? function _s(key) {
    ...
    value: function _binarySearh(list, word, _s) {
      ...
      while (left < right) {
        ...
        if (_common2.default.wordCompare(_s(word), _s(list[mid].keyText)) > 0) {
          left = mid + 1;
        } else if (_common2.default.wordCompare(_s(word), _s(list[mid].keyText)) == 0) {
          return mid;
        } else {
          right = mid - 1;
        }
