...
var Mdict = function (_MdictBase) {
  ...
  _createClass(Mdict, [{
    ...
    value: function _lookupKID(word) {
      ...
      var list = this._decodeKeyBlockByKBID(kbid);
      var i = this._binarySearh(list, word, sfunc);
      return { idx: i, list: list, kbid: kbid };
    }
  }, {
    key: "_binarySearh",
    ...
    value: function prefix(phrase) {
      ...
      return trie.commonPrefixSearch(phrase).map(function (item) {
        return { key: item.k, roffset: item.v };
      });
    }
    ...
    value: function parse_defination(word, roffset) {
      var rid = this._reduceRecordBlock(roffset);

      var _lookupKID3 = this._lookupKID(word),
          idx = _lookupKID3.idx,
          list = _lookupKID3.list,
          kbid = _lookupKID3.kbid;

      if (list[idx].recordStartOffset !== roffset) {

        var findIndex = function findIndex(list, roffset) {
          var i = void 0;
