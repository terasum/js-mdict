...
var MDictBase = /*#__PURE__*/function () {
  ...
  function MDictBase(fname, passcode) {
    ...
    this.ext = _common["default"].getExtension(fname, 'mdx'); // determine the encoding and decoder, if extension is *.mdd
  ...
  _createClass(MDictBase, [{
    ...
    value: function _readKeyHeader() {
      ...
      this._keyHeaderEndOffset = this._keyHeaderStartOffset + bytesNum + 4;
      /* 4 bytes adler32 checksum length */
    }
    /**
    ...
    value: function _splitKeyBlock(keyBlock) {
      ...
      while (keyStartIndex < keyBlock.length) {
        ...
        var recordStartOffset = _common["default"].readNumber(keyBlock.slice(keyStartIndex, keyStartIndex + this._numWidth), this._numFmt); // # key text ends with '\x00'


        var i = keyStartIndex + this._numWidth;

        while (i < keyBlock.length) {
          if (new _bl["default"](keyBlock.slice(i, i + width)).toString('hex') == delimiter) {
            keyEndIndex = i;
            break;
          }
