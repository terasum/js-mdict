...
var MDictBase = /*#__PURE__*/function () {
  ...
  function MDictBase(fname, passcode, options) {
    ...
    if (this.ext === "mdx" && this.options.resort) {
      ...
      if (this.options.debug) {
        console.time("KEY_LIST_RESORTE");
        console.log("file: ", this.fname);
        var _memTrace = _measureUtil["default"].measureMemFn();
        _memTrace("before resort decode");
        _memTrace("after resort decode");
        console.log("key entris (number): ", this.keyHeader.entriesNum);
        console.timeEnd("KEY_LIST_RESORTE");
      }
    }
  ...
  (0, _createClass2["default"])(MDictBase, [{
    ...
    key: "_stripKeyOrIngoreCase",
    value: function _stripKeyOrIngoreCase(key) {
      return function (key) {
