...
class MDict {
  constructor(fname, _passcode) {
    ...
    this.trie = _doublearray2.default.builder().build(this.key_data.map(keyword => (
  ...
  fuzzy_search(word, fuzzy_size, ed_gap) {
    const fuzzy_words = [];
    this.prefix(word).map(item => this._find_nabor(item.v, fuzzy_size).map(w => {
      const ed = _common2.default.levenshtein_distance(word, w.key);
      if (ed < (ed_gap || 5)) {
        fuzzy_words.push({
          ed,
          idx: w.idx,
  ...
  _find_nabor(sim_idx, fuzzy_size) {
    ...
    const sim_idx_start = sim_idx - fuzzy_size < 0 ? 0 : sim_idx - fuzzy_size;
    const sim_idx_end = sim_idx + fuzzy_size > set_size ? set_size : sim_idx + fuzzy_size;
   ...
   */
  _read_header() {
    const header_size_buffer = _readChunk2.default.sync(this.fname, 0, 4);
    const header_byte_size = _pythonStruct2.default.unpack(">I", header_size_buffer)[0];
    const header_b_buffer = _readChunk2.default.sync(this.fname, 4, header_byte_size);
    ...
    const header_text = _common2.default.readUTF16(header_b_buffer, 0, header_byte_size - 2);
    // parse header info
    ...
    if (!this.header_info.Encoding || this.header_info.Encoding == "") {
      ...
      this._encoding = this.header_info.Encoding.toLowerCase() == "utf16" || this.header_info.Encoding.toLowerCase() == "utf-16" ? UTF16 : UTF8;
  ...
  _read_keys() {
    ...
    const num_key_blocks = this.__readnumber(key_info_b.slice(key_i_ofst, key_i_ofst + this._number_width));
    key_i_ofst += this._number_width;
    // console.log("num_key_blocks", num_key_blocks.toString());

    // number of entries
    const num_entries = this.__readnumber(key_info_b.slice(key_i_ofst, key_i_ofst + this._number_width));
    key_i_ofst += this._number_width;
    ...
    if (this._version >= 2.0) {
      const key_block_info_decomp_size = this.__readnumber(key_info_b.slice(key_i_ofst, key_i_ofst + this._number_width));
      key_i_ofst += this._number_width;
    ...
    const key_block_info_size = this.__readnumber(key_info_b.slice(key_i_ofst, key_i_ofst + this._number_width));
    key_i_ofst += this._number_width;
    ...
    const key_blocks_total_size = this.__readnumber(key_info_b.slice(key_i_ofst, key_i_ofst + this._number_width));
    key_i_ofst += this._number_width;
    ...
    const key_block_info_list = this._decode_key_block_info(num_key_blocks, key_block_info, num_entries);
    ...
    (0, _assert2.default)(this.__toNumber(num_key_blocks) === key_block_info_list.length, "the num_key_info_list should equals to key_block_info_list");
    ...
    const key_block_compressed = new _bl2.default(this.__readfile(this.__toNumber(key_blocks_total_size)));
  ...
  _decode_key_block(key_block_compressed, key_block_info_list) {
    ...
    for (let idx = 0; idx < key_block_info_list.length; idx++) {
      ...
      const key_block_type = new _bl2.default(key_block_compressed.slice(start, start + 4));
      ...
      if (key_block_type.toString("hex") == "00000000") {
        ...
        const keyBlock = _lzo2.default.decompress(_appendBuffer(header, key_block_compressed.slice(start + 8, end)));
      ...
      key_list = key_list.concat(this._split_key_block(new _bl2.default(key_block), this._number_format, this._number_width, this._encoding));
      i += compressed_size;
    }
    return key_list;
  }
  ...
  _decode_record_block() {
    ...
    let size_counter = new _long2.default(0, 0);
    ...
    let item_counter = new _long2.default(0, 0);
    let record_offset = 0;
    // throw Error("ffff");
    for (let idx = 0; idx < record_block_info_list.length; idx++) {
      ...
      const record_block_compressed = new _bl2.default(this.__readfile(this.__toNumber(compressed_size)));
      // 4 bytes: compression type
      const record_block_type = new _bl2.default(record_block_compressed.slice(0, 4));
      ...
      if (record_block_type.toString("hex") === "00000000") {
        ...
        if (record_block_type.toString("hex") === "01000000") {
          ...
          record_block = _lzoWrapper2.default.decompress(_appendBuffer(header, blockBufDecrypted), decompressed_size, 1308672);
      ...
      (0, _assert2.default)(_numEqual(record_block.length, decompressed_size));
      ...
      while (i < this._key_list.length) {
        ...
        key_data.push({
          ...
          record_compressed_size: _long2.default.isLong(compressed_size) ? compressed_size.toNumber() : compressed_size,
          record_decompressed_size: _long2.default.isLong(decompressed_size) ? decompressed_size.toNumber() : decompressed_size,
  ...
  _split_key_block(key_block, _number_format, _number_width, _encoding) {
    ...
    while (key_start_index < key_block.length) {
      ...
      const key_id = _pythonStruct2.default.unpack(_number_format, key_block.slice(key_start_index, key_start_index + _number_width))[0];
      ...
      const key_text = this._decoder.decode(key_block.slice(key_start_index + _number_width, key_end_index));
  ...
  _decode_key_block_info(num_key_blocks, key_info_bl, num_entries) {
    ...
    (0, _assert2.default)(_numEqual(this.header_info.key_block_info_decomp_size, key_block_info.length), "key_block_info length should equal");
    const key_block_info_list = [];

    let count_num_entries = new _long2.default(0x00000000, 0x00000000);
    let byte_format = ">H";
    let byte_width = 2;
    let text_term = 1;
    if (this._version >= 2.0) {
    ...
    let count = 0;
    let i = 0;
    const key_block_num = _long2.default.isLong(num_key_blocks) ? num_key_blocks.toNumber() : num_key_blocks;
    while (count < key_block_num) {
      // number of entries in current key block

      const tmp_count = _pythonStruct2.default.unpack(this._number_format, key_block_info.slice(i, i + this._number_width))[0];
      count_num_entries = count_num_entries.add(tmp_count);
      i += this._number_width;

      const first_key_size = _pythonStruct2.default.unpack(byte_format, key_block_info.slice(i, i + byte_width))[0];
      i += byte_width;
      ...
      // text tail
      const keyblock_last_key_len = _pythonStruct2.default.unpack(byte_format, key_block_info.slice(i, i + byte_width))[0];
      i += byte_width;
      if (this._encoding === UTF16) {
      ...
      const key_block_compressed_size = _pythonStruct2.default.unpack(this._number_format, key_block_info.slice(i, i + this._number_width))[0];
      i += this._number_width;
      // key block decompressed size
      const key_block_decompressed_size = _pythonStruct2.default.unpack(this._number_format, key_block_info.slice(i, i + this._number_width))[0];
      i += this._number_width;
  ...
  __mdx_decrypt(comp_block) {
    const key = _ripemd2.default.ripemd128(new _bl2.default(comp_block.slice(4, 8)).append(_pythonStruct2.default.pack("<L", 0x3695)).slice(0, 8));
  ...
  __readfile(length) {
    const b = _readChunk2.default.sync(this.fname, this.offset, length);
    this.offset += length;
    return new _bl2.default().append(b);
  }

  __readbuffer(start, length) {
    const buf = _readChunk2.default.sync(this.fname, start, length);
