...
var MDict = function () {
  function MDict(fname, _passcode) {
    ...
    this.trie = _doublearray2.default.builder().build(this.key_data.map(function (keyword) {
      return (
  ...
  _createClass(MDict, [{
    ...
    key: "_read_keys",
    value: function _read_keys() {
      var num_bytes = void 0;
      if (this._version >= 2.0) {
        num_bytes = 8 * 5;
      } else {
        num_bytes = 4 * 4;
      }
      var key_info_b = this.__readfile(num_bytes);
      ...
      var num_key_blocks = this.__readnumber(key_info_b.slice(key_i_ofst, key_i_ofst + this._number_width));
      key_i_ofst += this._number_width;
      // console.log("num_key_blocks", num_key_blocks.toString());

      // number of entries
      var num_entries = this.__readnumber(key_info_b.slice(key_i_ofst, key_i_ofst + this._number_width));
      key_i_ofst += this._number_width;
      ...
      if (this._version >= 2.0) {
        var key_block_info_decomp_size = this.__readnumber(key_info_b.slice(key_i_ofst, key_i_ofst + this._number_width));
        key_i_ofst += this._number_width;
      ...
      var key_block_info_size = this.__readnumber(key_info_b.slice(key_i_ofst, key_i_ofst + this._number_width));
      key_i_ofst += this._number_width;
      ...
      var key_blocks_total_size = this.__readnumber(key_info_b.slice(key_i_ofst, key_i_ofst + this._number_width));
      key_i_ofst += this._number_width;
      ...
      var key_block_info = this.__readfile(this.__toNumber(key_block_info_size));
      // console.log(key_block_info.slice(0, 20).toString("hex"));
      var key_block_info_list = this._decode_key_block_info(num_key_blocks, key_block_info, num_entries);
      ...
      var key_block_compressed = new _bl2.default(this.__readfile(this.__toNumber(key_blocks_total_size)));

      // -------- 到这里为止都是正确的


      // console.log("key_block_info_list", key_block_info_list);
      var key_list = this._decode_key_block(key_block_compressed, key_block_info_list);
    ...
    value: function _decode_key_block(key_block_compressed, key_block_info_list) {
      ...
      for (var idx = 0; idx < key_block_info_list.length; idx++) {
        ...
        if (key_block_type.toString("hex") == "00000000") {
          ...
          var keyBlock = _lzo2.default.decompress(_appendBuffer(header, key_block_compressed.slice(start + 8, end)));
    ...
    value: function _decode_record_block() {
       ...
       */
      var num_record_blocks = this.__readnumber(this.__readfile(this._number_width));
      var num_entries = this.__readnumber(this.__readfile(this._number_width));
      (0, _assert2.default)(_numEqual(num_entries, this._num_entries));

      var record_block_info_size = this.__readnumber(this.__readfile(this._number_width));
      var record_block_size = this.__readnumber(this.__readfile(this._number_width));
      /**
      ...
      for (var _i = 0; _i < num_record_blocks; _i++) {
        var compressed_size = this.__readnumber(this.__readfile(this._number_width));
        var decompressed_size = this.__readnumber(this.__readfile(this._number_width));
      ...
      for (var idx = 0; idx < record_block_info_list.length; idx++) {
        ...
        var record_block_compressed = new _bl2.default(this.__readfile(this.__toNumber(_compressed_size)));
        // 4 bytes: compression type
    ...
    value: function _decode_key_block_info(num_key_blocks, key_info_bl, num_entries) {
      ...
      while (count < key_block_num) {
        ...
        var tmp_count = _pythonStruct2.default.unpack(this._number_format, key_block_info.slice(i, i + this._number_width))[0];
        ...
        var key_block_compressed_size = _pythonStruct2.default.unpack(this._number_format, key_block_info.slice(i, i + this._number_width))[0];
        i += this._number_width;
        // key block decompressed size
        var key_block_decompressed_size = _pythonStruct2.default.unpack(this._number_format, key_block_info.slice(i, i + this._number_width))[0];
        i += this._number_width;
    ...
    key: "__readfile",
    value: function __readfile(length) {
    ...
    key: "__readnumber",
    value: function __readnumber(bf) {
