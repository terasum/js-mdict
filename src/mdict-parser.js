...
function parse_mdict(file, ext) {
  ...
  let attrs = {}, // storing dictionary attributes
  ...
  function config() {
    ...
    if (isTrue(attrs.KeyCaseSensitive)) {
      this._adaptKey = isTrue(attrs.StripKey)
        ? (key) =>  { key.replace(regexp, "$1"); }
        : (key) =>  { key }
    } else {
      this._adaptKey = isTrue(attrs.StripKey || (_v2 ? "" : "yes")) ?
        function (key) {
  ...
  function Scanner(buf, len) {
    ...
    const methods = {
      ...
      readBlock(len, expectedBufSize, decryptor) {
        const comp_type = dv.getUint8(offset, false); // compression type, 0 = non, 1 = lzo, 2 = gzip
        if (comp_type === 0) {
          if (_v2) {
  ...
  function read_record_summary(input, pos) {
    console.log("[o] record section offset", pos);
    let scanner = Scanner(input),
      record_summary = {
        num_blocks: scanner.readNum(),
        num_entries: scanner.readNum(),
    ...
    console.log("[o] record summary", record_summary);
  ...
  function read_record_block(input, record_summary) {
    ...
    RECORD_BLOCK_TABLE.put(p0, p1);
    console.log("[o] record_index: ", record_index[record_index.length - 1]);
  }
  ...
  function findWord(keyinfo) {
    ...
    return _slice(block.comp_offset, block.comp_size)
      ...
      .spread(definition => resolve(followLink(definition, LOOKUP.mdx)));
  }
  ...
  function appendMore(word, list, nextKdx, expectedSize, filter, ticket) {
    ...
    if (filter) {
      if (_trail.count < expectedSize && nextKdx && nextKdx.first_word.substr(0, word.length) === word) {
        return loadKeys(nextKdx).delay(30).then((more) => {
          _trail.offset = 0;
    ...
    const shortage = expectedSize - list.length;
    if (shortage > 0 && nextKdx) {
      _trail.block = nextKdx.index;
      return loadKeys(nextKdx).then((more) => {
        _trail.offset = 0;
  ...
  var LOOKUP = {
      ...
      if (typeof query === "string" || query instanceof String) {
        ...
        return seekVanguard(word).spread((kdx, idx, list) => {
          ...
          if (offset !== UNDEFINED) {
            ...
            list = list.filter(el => el.toLowerCase() === word);
          }
    ...
    mdd(phrase) {
      ...
      word = `\\${word.replace(/(^[/\\])|([/]$)/, "")}`;
      word = word.replace(/\//g, "\\");
      return seekVanguard(word).spread((kdx, idx, list) => list.slice(idx).filter(one => one.toLowerCase() === word)).then((candidates) => {
  ...
  return _slice(pos, 4).exec(read_file_head).then((len) => {
    ...
    //  4 + input(header_sect(len) + 48)
    // 4 => header_length
    // header_length => header_sect
    // header_length + 48 => input
    // header_length + 4 => remain_len
    //
    ...
    .spread((record_summary) => {
      ...
      console.log("[o] record block start offset", pos);
