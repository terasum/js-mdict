...
class MDictBase {
  ...
  constructor(
    ...
    if (this.options.debug) {
      console.timeLog('constructor', 'readHeader finished');
      console.log('readHeader finished', { header: this.header });
    }
    ...
    if (this.options.debug) {
      console.timeLog('constructor', 'readKeyHeader finished');
    ...
    if (this.options.debug) {
      console.timeLog('constructor', 'readKeyBlockInfo finished');
      console.log('readKeyBlockInfo finished', {
        keyBlockInfoList: this.keyBlockInfoList,
      });
    }
    ...
    if (this.options.debug) {
      ...
      memTrace('before _decodeKeyBlock');
      console.time('_decodeKeyBlock');
      console.timeLog('_decodeKeyBlock', 'start');
      measure.measureTimeFn(this, this._decodeKeyBlock)();
      console.timeLog('_decodeKeyBlock', 'end');
      console.timeEnd('_decodeKeyBlock');
      // memTrace('after _decodeKeyBlock');
      console.time('_resortKeyList');
      this._resortKeyList();
      // measure.measureTimeFn(this, this._resortKeyList)();
      console.timeLog('_resortKeyList', 'end');
      console.timeEnd('_resortKeyList');
    ...
    if (this.options.debug) {
      console.timeLog('constructor', 'decodeRecordHeader finished');
    ...
    if (this.options.debug) {
      console.timeLog('constructor', 'decodeRecordInfo finished');
      console.log('decodeRecordInfo finished', {
        recordBlockInfoList: this.recordBlockInfoList,
      });
    }
    ...
    // console.timeLog("constructor", "decodeRecordBlock start")
    // this._decodeRecordBlock();
    // console.timeLog("constructor", "decodeRecordBlock end")
   ...
   */
  _decodeKeyBlock() {
    console.timeLog('_decodeKeyBlock', 'before read buffer', {
    ...
    console.timeLog('_decodeKeyBlock', 'read buffer sample', {
      kbCompBuff: kbCompBuff.subarray(0, 100),
    });
    let key_list: KeyListItem[] = [];
    let kbStartOfset = 0;
    console.timeLog('_decodeKeyBlock', 'start decode key block', {
      keyBlocksTotalSize: this.keyHeader.keyBlocksTotalSize,
      kbCompBuff: kbCompBuff.subarray(0, 100),
    ...
    for (let idx = 0; idx < this.keyBlockInfoList.length; idx++) {
      ...
      console.timeLog('_decodeKeyBlock', 'decode key block', {
        idx,
        compSize,
        decompressedSize,
        start,
      ...
      if (kbCompType.toString('hex') == '00000000') {
        ...
        console.timeLog('_decodeKeyBlock', 'decompress key block', {
          header,
          kbCompBuff: kbCompBuff.subarray(start + 8, end),
        });

        const combinedKbCompressedBuff = common.appendBuffer(header, kbCompBuff.subarray(start + 8, end));
        // console.log("============");
      ...
      console.timeLog('_decodeKeyBlock', 'split key block', {
        key_block: key_block.subarray(0, 100),
        compressType: kbCompType.toString('hex'),
      });
      ...
      if (this.options.debug) {
        console.timeLog('_decodeKeyBlock key_list', { key_list });
      }
    }
  ...
  _decodeKeyBlockByKBID(kbid: number): KeyListItem[] {
    ...
    const start = 0;
    const end = compSize;
    const kbCompType = Buffer.from(kbCompBuff.slice(start, start + 4));
    // TODO 4 bytes adler32 checksum
    // # 4 bytes : adler checksum of decompressed key block
    // adler32 = unpack('>I', key_block_compressed[start + 4:start + 8])[0]
    ...
    if (kbCompType.toString('hex') == '00000000') {
      ...
      key_block = Buffer.from(keyBlock).slice(
        keyBlock.byteOffset,
  ...
  _decodeRecordHeader(): void {
    if (this.options.debug) {
      console.timeLog('_decodeRecordHeader');
    }
    ...
    if (this.options.debug) {
      console.timeLog('_decodeRecordHeader rhBuffer', { rhBuff: rhBuff });
    }
    let ofset = 0;
    ...
    if (this.options.debug) {
      console.timeLog('_decodeRecordHeader recordBlocksNum', {
        recordBlocksNum,
      });
    }
    ofset += this._numWidth;
    ...
    if (this.options.debug) {
      console.timeLog('_decodeRecordHeader entriesNum', { entriesNum });
    }
    ofset += this._numWidth;
    ...
    if (this.options.debug) {
      console.timeLog('_decodeRecordHeader recordBlockInfoCompSize', {
    ...
    if (this.options.debug) {
      console.timeLog('_decodeRecordHeader recordBlockCompSize', {
        recordBlockCompSize,
      });
    }
    this.recordHeader = {
      ...
      if (rbCompType.toString('hex') === '00000000') {
        ...
        if (rbCompType.toString('hex') === '01000000') {
          ...
          recordBlock = Buffer.from(
            lzo1x.decompress(common.appendBuffer(header, blockBufDecrypted),
            decompSize,
            1308672
          ));
   ...
   */
  _reduceRecordBlock(recordStart: number): number {
    let left = 0;
